function res = dtd_pa_mkpdf(paths,opt)
% function res = dtd_mkpdf(paths,opt)
 
res = -1;

mfs = mdm_mfs_load(paths.mfs.dtd_pa_primary_fn);
sz = size(mfs.m);
mfs_derived = mdm_mfs_load(paths.mfs.dtd_pa_derived_fn);
...
eval(['print ' paths.maps '/dtd_pa -loose -dpdf'])

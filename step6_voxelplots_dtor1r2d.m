...
% method = 'dtr1r2d'; omega_v = [150 50]*2*pi; omega_v = linspace(50,150,5)*2*pi;
...
[dpar_4d,dperp_4d,theta_4d,phi_4d,d0_4d,rpar_4d,rperp_4d,r1_4d,r2_4d,w_4d] = dtor1r2d_bsmerge_initializepars;
m_bsmerge = [];
for nbs = 1:numel(bsno)
    ...
    if exist(mfs_fn,'file')==2
        ...
        [dpar_temp,dperp_temp,theta_temp,phi_temp,d0_temp,rpar_temp,rperp_temp,r1_temp,r2_temp,w_temp] = dtor1r2d_4d_m2pars(m);
        [dpar_4d,dperp_4d,theta_4d,phi_4d,d0_4d,rpar_4d,rperp_4d,r1_4d,r2_4d,w_4d] = dtor1r2d_bsmerge_catpars(dpar_4d,dperp_4d,theta_4d,phi_4d,d0_4d,rpar_4d,rperp_4d,r1_4d,r2_4d,w_4d,dpar_temp,dperp_temp,theta_temp,phi_temp,d0_temp,rpar_temp,rperp_temp,r1_temp,r2_temp,w_temp);
    end
end
...
map_nams = {'s0'; 'fractions'; 'mdiso'; 'msddelta'; 'vdiso';'dmdisodnu';'mdii_bin1'};
map_nams = {'s0'};
...
for nmap = 1:Nmaps
    ...
    if sz(1) == 3
        if length(sz) == 3
            I = reshape(I,[3 sz(2) sz(3) 1]);
        end
    else
        if length(sz) == 2
            I = reshape(I,[sz(1) sz(2) 1]);
        end
    end
...
imaspect = sz(2)/sz(1);
papersize_i = 5*[Nmaps/opt.mplot.zoomx 1*pixaspect*imaspect/opt.mplot.zoomy];
papersize_p = [10 5*2];
papersize_s = [4 4];
papersize_d = [4 4];
pformat = '-dpdf';
...
for nmap = 1:Nmaps
    ...
    axis(axh,'equal','tight','off')
    set(axh,'XLim',sz(1)/2 + 1/opt.mplot.zoomx*sz(1)/2*[-1 1],'YLim',sz(2)/2 + 1/opt.mplot.zoomy*sz(2)/2*[-1 1])
    hold on
...
eval(['print ' voxel_path '/labeledvoxels_slice_' num2str(nk) ' -loose -dpng'])
...
xps_array = [round(xps.tr,3),round(xps.te,3), round(xps.b,-7), round(xps.b_delta,1), xps.theta/pi*180, xps.phi/pi*180];
[xps_array_sort, ind_sort] = sortrows(xps_array,[1 2 3 4 5 6]);
ind_sort = 1:xps.n;
...
axh_bd = axes('position',[left bottom+5*dbottom width height]);
ph_bd = plot(1:xps.n,xps.b_delta(ind_sort),'.');
axh_btheta = axes('position',[left bottom+4*dbottom width height]);
ph_btheta = plot(1:xps.n,acos(xps.u((ind_sort),3))/pi*180,'.');
axh_bphi = axes('position',[left bottom+3*dbottom width height]);
ph_bphi = plot(1:xps.n,atan2(xps.u((ind_sort),2),xps.u((ind_sort),1))/pi*180,'.');
axh_omega = axes('position',[left bottom+2*dbottom width height]);
ph_omega = plot(1:xps.n,xps.rmsomega(ind_sort)/2/pi,'.');
axh_tr = axes('position',[left bottom+1*dbottom width height]);
ph_tr = plot(1:xps.n,xps.tr(ind_sort)*1e3,'.');
axh_te = axes('position',[left bottom+0*dbottom width height]);
ph_te = plot(1:xps.n,xps.te(ind_sort)*1e3,'.');

set([axh_b; axh_bd; axh_btheta; axh_bphi; axh_omega; axh_tr; axh_te],...
'XLim',xps.n*[-.05 1.05],'Box','off','TickDir','out','TickLength',.01*[1 1],'LineWidth',lw,'FontSize',fs)    
set([axh_b; axh_bd; axh_btheta; axh_bphi; axh_omega; axh_tr],'XTickLabel',[])    
set([ph_b; ph_bd; ph_btheta; ph_bphi; ph_omega; ph_tr; ph_te],'LineWidth',.5,'Color','k')    
...
set(axh_tr,'YLim',max(xps.tr*1e3)*[-.05 1.05])    
set(axh_te,'YLim',max(xps.te*1e3)*[-.05 1.05])    
...
%     hold([hax; hax_projx; hax_projy],'on')
    ...
    dtor1r2d = dtor1r2d_par2dist(squeeze(dpar_4d(ni,nj,nk,:)),squeeze(dperp_4d(ni,nj,nk,:)),squeeze(theta_4d(ni,nj,nk,:)),...
        squeeze(phi_4d(ni,nj,nk,:)),squeeze(d0_4d(ni,nj,nk,:)),squeeze(rpar_4d(ni,nj,nk,:)),squeeze(rperp_4d(ni,nj,nk,:)),...
        squeeze(r1_4d(ni,nj,nk,:)),squeeze(r2_4d(ni,nj,nk,:)),...
        squeeze(w_4d(ni,nj,nk,:)));
    for nomega = 1:numel(omega_v)
        [n,dparo,dperpo,theta,phi,r1,r2,w] = dtor1r2d_dist2parso(dtor1r2d,omega_v(nomega));
...
%         axpars.xmin = -11; axpars.xmax = -8; axpars.ymin = -3; axpars.ymax = 3; 
        axpars.xmin = -.1; axpars.xmax = 2.6; axpars.ymin = -.1; axpars.ymax = 1.1; 
        axpars.no_scatter = 1;

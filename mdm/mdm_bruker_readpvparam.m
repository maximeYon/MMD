function paramValue = mdm_bruker_readpvparam(fidPath,paramName,TwoDSeqPath)
%function paramValue=ReadPVParam(fidPath,paramName,TwoDSeqPath)
%
%read a parameter value from the parameter files (method, acqp, subject, and reco).
%
...
% if(isempty(paramName) || isempty(fidPath))
    ...
    %path to the method and acqp  and subject files
    methodPath=fullfile(fidPath,'method');
    acqpPath=fullfile(fidPath,'acqp');
    subjectPath= fullfile(fileparts(fidPath),'subject');
...
%         % Value is a string
%         if numel(indxs) == 1
%             paramValue = tempval(indxs(1)+1:length(tempval));
%         else
%             paramValue = tempval(indxs(1)+1:indxs(2)-1);
%         end
%     end
    paramValue = tempval;
end

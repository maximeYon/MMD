function [ADC,FA,reD,greeN,bluE,VectorF,DifT] = DTI_EPI_PV_NIFTI(data_path)
...
parameters = struct;
parameters.BackgroundTreshold = 0; % 0.05
parameters.WhiteMatterExtractionThreshold = 0.0; %0.05;
parameters.textdisplay = false;

%% Diffusion Tensor Image (DTI) calculations
S = squeeze(data);
for exp = 1:size(GradientOrientations,1)
    DTIdata(exp).VoxelData = S(:,:,:,exp);
    DTIdata(exp).Gradient = GradientOrientations(exp,:);
    DTIdata(exp).Bvalue = DwEffBval(1,exp);
end
[ADC,FA,reD,greeN,bluE,VectorF,DifT] = my_DTI(DTIdata,parameters);

end


function [ADC,FA,reD,greeN,bluE,VectorF,DifT]=my_DTI(DTIdata,parameters)
...
% Create a matrix to store RGB matrix for rgb figure
reD=zeros(size(S0),'single');
greeN=zeros(size(S0),'single');
bluE=zeros(size(S0),'single');
    ...
    for y=1:size(S0,2)
        for z=1:size(S0,3)
            ...
            if(S0(x,y,z)>parameters.BackgroundTreshold)
                ...
                reD(x,y,z)=abs(EigenVectors(1,3));
                greeN(x,y,z)=abs(EigenVectors(3,3));
                bluE(x,y,z)=abs(EigenVectors(2,3));
            end
        end
    end
end

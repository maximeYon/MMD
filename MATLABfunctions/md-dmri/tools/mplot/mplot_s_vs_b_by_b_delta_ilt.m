function m = mplot_s_vs_b_by_b_delta_ilt(S, xps, fun_data2fit, fun_fit2data, opt, h, h2, ind)
...
for c = 1:numel(b_delta_uni)
    ...
    if (~isempty(fun_data2fit))
        m = fun_data2fit(S(ind2), mdm_xps_subsample(xps, ind2), opt);
        m_list{c} = m;
    else
        m = [];
        m_list{c} = m;
...
switch (method_name)
    ...
    case 'ilt'
        ...
        for c = 1:numel(b_delta_uni)
            ...
            idx_w_pos = w > 0;
            w_list{c} = w(idx_w_pos);
            D_list{c} = D(idx_w_pos);
        end
...
if (~isempty(w)) && strcmp(method_name,'ilt')
    for c = 1:numel(b_delta_uni)
        %D = D_list{c};
        %w = w_list{c};
        mplot_dd(D_list, w_list, color_list, opt.ilt.dmin, opt.ilt.dmax, h2, opt);
    end
else
    axis(h2, 'off');
end

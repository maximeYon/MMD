...
if isstruct(im3d)
    ...
    axh_v = [];
    for k = 1:sz(3)
        im2d = zeros(sz(2),sz(1),3); %Initialize 2D image with RGB in dimension 3
        im2d(:,:,1) = im3d.r(:,:,k)'; %Red
        im2d(:,:,2) = im3d.g(:,:,k)'; %Green
        im2d(:,:,3) = im3d.b(:,:,k)'; %Blue
        im2d = im2d./repmat(max(im2d,[],3),[1 1 3]).*repmat(im3d.bright(:,:,k)',[1 1 3]); %Color normalization [R,G,B]/max(R,G,B) and scaling with brightness

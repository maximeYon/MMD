...
opt.dtr2d.dmax = 1/min(xps.b);
opt.dtr2d.r2min = .1/max(xps.te);
opt.dtr2d.r2max = 10/min(xps.te);
opt.dtr2d.r2reg = 0e-4;
[opt.dtr2d.r2max opt.dtr2d.r2reg]
...
% stemp = S;
% r2reg = 1;
% maxk = max(exp(-te*r2'),[],1);
% A = r2reg*numel(stemp)*(1-maxk)
% snorm = max(stemp);
% 
% chisq = sum((S_fit/snorm-S/snorm).^2)
% chisqreg = (A*w/snorm)^2

misfitlevel = sqrt(mean((S_fit-S).^2));

cla(axh);
hold(axh, 'off');
h1 = plot(axh,1:xps.n,S,'ro',1:xps.n,S_fit,'k.',[1; xps.n],misfitlevel*[1; 1],'k--');

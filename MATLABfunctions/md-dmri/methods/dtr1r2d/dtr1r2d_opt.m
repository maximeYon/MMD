...
opt.(method) = msf_ensure_field(opt.(method), 'fig_maps', ...
    {'s0','mdiso','msddelta','mr1','mr2','vdiso','vsddelta','vr1','vr2'});
opt.(method) = msf_ensure_field(opt.(method), 'fig_prefix', 'dtr1r2d');
...
opt.(method) = msf_ensure_field(opt.(method), 'bin_r1max',opt.(method).r1max*[1; 1; 1]);
opt.(method) = msf_ensure_field(opt.(method), 'bin_r1min',opt.(method).r1min*[1; 1; 1]);
...
opt.(method) = msf_ensure_field(opt.(method), 'do_r1r2switch', 0); % Switch from Reymbaut dtr2r1d to dtr1r2d

...
dps.s0 = sum(dtds.w,4);
dps.s_b0500 = msf_notfinite2zero(sum(exp(-.5e9*dtds.diso).*dtds.w,4));
dps.s_b1000 = msf_notfinite2zero(sum(exp(-1e9*dtds.diso).*dtds.w,4));
dps.s_b2000 = msf_notfinite2zero(sum(exp(-2e9*dtds.diso).*dtds.w,4));
dps.s_b3000 = msf_notfinite2zero(sum(exp(-3e9*dtds.diso).*dtds.w,4));
dps.s_b4000 = msf_notfinite2zero(sum(exp(-4e9*dtds.diso).*dtds.w,4));
dps.s_b5000 = msf_notfinite2zero(sum(exp(-5e9*dtds.diso).*dtds.w,4));
dps.s_b0500_bdelta1 = msf_notfinite2zero(sum(dtd_pake_parsv2signalv(dtds, .5e9, 1).*dtds.w,4));
dps.s_b1000_bdelta1 = msf_notfinite2zero(sum(dtd_pake_parsv2signalv(dtds, 1e9, 1).*dtds.w,4));
dps.s_b2000_bdelta1 = msf_notfinite2zero(sum(dtd_pake_parsv2signalv(dtds, 2e9, 1).*dtds.w,4));

%For comparison with EUSOBI consensus ADC, see https://doi.org/10.1007/s00330-019-06510-3
dps.s_b0800 = msf_notfinite2zero(sum(exp(-0.8e9*dtds.diso).*dtds.w,4));
dps.adc800 = msf_notfinite2zero((log(dps.s0) - log(dps.s_b0800))./ 0.8e9)/1e-9;

...
opt.(method).rmin = .01*min(xps.rmsomega(xps.rmsomega>0)/2/pi);
opt.(method).rmax = 100*max(xps.rmsomega/2/pi);
...
title(axh,['\omega_{rms} / 2\pi = ' num2str(min(xps.rmsomega(xps.rmsomega>0)/2/pi),3) ' - ' num2str(max(xps.rmsomega/2/pi),3) ' Hz'])

% Get dps from dtor1r2d_4d_fit2param
mfs.m = zeros(1,1,1,numel(m)); mfs.m(1,1,1,:) = m;
dps = dtor1r2d_4d_fit2param(mfs.m,[],opt);

[dpar,dperp,theta,phi,w] = dtor1r2d_4d_m2parso(mfs.m,mean(xps.rmsomega));
[dxx,dyy,dzz,dxy,dxz,dyz] = dtd_pars2elements(squeeze(dpar),squeeze(dperp),squeeze(theta),squeeze(phi));
dtd_1x6 = [dxx dyy dzz sqrt(2)*[dxy dxz dyz]];

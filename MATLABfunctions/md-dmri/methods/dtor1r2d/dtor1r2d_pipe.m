function fn = dtor1r2d_pipe(s, paths, opt)
% function fn = dtor1r2d_pipe(s, paths, opt)
%
% s     - input structure
...
if (opt.do_data2fit)
    if opt.do_bootstrap
        ...
        ind = (opt.dtor1r2d.ind_start-0) + round(rand([s.xps.n-(opt.dtor1r2d.ind_start-1),1])*(s.xps.n-(opt.dtor1r2d.ind_start-0)));
        save(paths.ind_fn, 'ind');
        ind_fn = mdm_ind_save(ind, paths.ind_fn);
        load(paths.ind_fn)
        opt.bootstrap.ind = ind;
    end
    mdm_data2fit(@dtor1r2d_4d_data2fit, s, paths.mfs_fn, opt);
...
if (opt.do_datafit2chisq)
    opt.dtd.ind_start = opt.dtor1r2d.ind_start;
    chisq_fn = mio_datafit2chisq(@dtor1r2d_1d_fit2data, s, paths.mfs_fn, paths.chisq_fn, opt);
end

if (opt.do_fit2param)
    mdm_fit2param(@dtor1r2d_4d_fit2param, paths.mfs_fn, paths.dps_fn, opt);
end

% Save nifti parameter maps    
if (opt.do_param2nii)
    fn = mdm_param2nii(paths.dps_fn, paths.nii_path, opt.dtor1r2d, opt);
end

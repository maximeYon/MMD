...
% [b_ind,bd_ind,gwf_ind,br_ind] = ndgrid(1:n_b,1:n_bd,1:n_gwf,1:n_br);
%[b_ind,bd_ind,br_ind,gwf_ind] = ndgrid(1:n_b,1:n_bd,1:n_br,1:n_gwf);
[b_ind,bd_ind,br_ind,gwf_ind] = ndgrid(1:n_b,1:n_bd,1:n_br,n_gwf:-1:1);
xps.n = numel(b_ind);
...
xps.gwf_ind = reshape(gwf_ind,xps.n,1); % Same gwf_ind means same waveform
xps.br_ind = reshape(br_ind,xps.n,1); % Same br_ind means same b-tensor orientation
% [a_ind,~] = ndgrid(1:(n_b*n_bd*n_gwf),1:n_br);
% xps.a_ind = reshape(a_ind,xps.n,1); % Same a_ind means same b-tensor size and shape

if randomised ==1
    ind = randperm(xps.n)';
    Grel_tmp = sqrt(brel(xps.b_ind));
    if ~isempty(find(xps.gwf_ind==0))% if short TE experiment
        first_ind = find(xps.gwf_ind==0);
        first_ind = first_ind(1,1);
        ind = ind(ind~=first_ind);
        ind = [first_ind; ind];
    else                      % find a b0
        first_ind = find(Grel_tmp==0);
        first_ind = first_ind(1,1);
        ind = ind(ind~=first_ind);
        ind = [first_ind; ind]; 
    end
...
% fields = {'b_ind','bd_ind','gwf_ind','br_ind','a_ind'};
fields = {'b_ind','bd_ind','gwf_ind','br_ind'};
for nfield = 1:numel(fields)
    field = fields{nfield};
    xps.(field) = xps.(field)(ind);
end
...
subplot(3,1,3)
ha = plot(1:xps.n,xps.gwf_ind,'ko');
...
% Write list
list_file = fopen([out_path, filesep,'DORlist_',num2str(list_size)],'w');

...
clearvars;close all;
xlsx_name = 'DOR_protocol_invivo_1shape_393_lowB'; % 'List_testBvalue'
...
out_path = 'C:\Users\User\Mon_Drive\Matlab\MATLABfunctions\md-dmri\acq\bruker\Paravision360\7T_Kuopio\DOR_list\';
% out_path = 'C:\Users\Administrateur\Mon_Drive\Matlab\MATLABfunctions\md-dmri\acq\bruker\Paravision360\DOR_list';
...
multislice = 1; % if multisclice =1, sorting is different to have a short TR as first experiment
...
param.EffiFactor = [0.00531892472259935,0.00266418742026941,0.00160509321854446,0.000833283616846146,0.000499684111434804,0.000319856797483992]; % 500 points
% tau = 4;
% bval = (2*param.EffiFactor*(1.*param.Gmax)^2*(tau*10^-3)^3*gamma1H^2)*10^-9;
param.Min_Shape_Dur = 6; % minimal duration of shapes in ms
...
%% Grel normalization by maximum order shape
max_shape_order = 0;
for c = 1:size(CellParam,2)
    max_shape_order = max([max_shape_order max(CellParam{6,c})]);
end
...
% Write list
if multislice ==1
    list_file = fopen([out_path, filesep,'DOR_R1R2_',num2str(list_size) '_',num2str(max_shape_order) '_ms'],'w');
else
    list_file = fopen([out_path, filesep,'DOR_R1R2_',num2str(list_size) '_',num2str(max_shape_order)],'w');
end

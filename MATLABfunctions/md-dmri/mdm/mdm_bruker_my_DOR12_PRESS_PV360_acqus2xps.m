function xps = mdm_bruker_my_DOR12_PRESS_PV360_acqus2xps(data_path, xps_fn, rps)
...
u1030 = 0.001030;
EffecectiveTRcalc = d1+u1030+AddTRList+d4+d6+(p1*(1-VoxPul1_Rpfac));
...
Ndt_preref = Ndt_p1half + Ndt_d6 + Ndt_d7 + Ndt_d24 + Ndt_d7 + ...
    Ndt_AddTE + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList  +...
    Ndt_d8 + Ndt_d4 + Ndt_d6 + Ndt_p3half;
Ndt_postref = Ndt_p3half + Ndt_d6 + Ndt_d8 + Ndt_d4 +...
    (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE +  ...
    Ndt_d9 + Ndt_d24 + Ndt_d7 + Ndt_u20 + Ndt_d10 + Ndt_d4 + Ndt_d6 + Ndt_p4half + ...
    Ndt_p4half + Ndt_d6 + Ndt_d10 + Ndt_d11 + Ndt_u10 + Ndt_u10; % stop before aqq if FID (not echo acquistion)
Ndt_tot = Ndt_preref + Ndt_postref;

%% Create sequences part for b-calculation
Seq_part(1,:) = Ndt_p1half + Ndt_d6 + Ndt_d7 + Ndt_d24 + Ndt_d7 + Ndt_AddTE;
Seq_part(2,:) = Seq_part(1,:) + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList;
Seq_part(3,:) = Seq_part(2,:) + Ndt_d8 + Ndt_d4 + Ndt_d6 + Ndt_p3half + Ndt_p3half + Ndt_d6 + Ndt_d8 + Ndt_d4 ;
Seq_part(4,:) = Seq_part(3,:) + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE;
Seq_part(5,:) = Seq_part(4,:) + Ndt_d9 + Ndt_d24 + Ndt_d7 + Ndt_u20 + Ndt_d10 + Ndt_d4 + Ndt_d6 + Ndt_p4half + Ndt_p4half + Ndt_d6 + Ndt_d10 + Ndt_d11 + Ndt_u10 + Ndt_u10;
...
g24_recalc = -(g1*(Ndt_p1half+Ndt_d6+Ndt_d7/2))/Ndt_d24;
G.exc_x = [g1*ones(Ndt_p1half,1);g1*ones(Ndt_d6,1); g1/2*ones(Ndt_d7,1); g24_recalc*ones(Ndt_d24,1); 0*ones(Ndt_d7,1)]*unit2si;
G.exc_y = [0*ones(Ndt_p1half,1);0*ones(Ndt_d6,1); 0*ones(Ndt_d7,1); 0*ones(Ndt_d24,1); 0*ones(Ndt_d7,1)]*unit2si;
G.exc_z = [0*ones(Ndt_p1half,1);0*ones(Ndt_d6,1); 0*ones(Ndt_d7,1); 0*ones(Ndt_d24,1); 0*ones(Ndt_d7,1)]*unit2si;

G.ref_x = [g4*ones(Ndt_d8,1);0*ones(Ndt_d4,1);0*ones(Ndt_d6,1); 0*ones(Ndt_p3half,1); 0*ones(Ndt_p3half,1);0*ones(Ndt_d6,1);g5*ones(Ndt_d8,1);0*ones(Ndt_d4,1)]*unit2si;
G.ref_y = [g6*ones(Ndt_d8,1);g2/2*ones(Ndt_d4,1);g2*ones(Ndt_d6,1);g2*ones(Ndt_p3half,1);g2*ones(Ndt_p3half,1);g2*ones(Ndt_d6,1);g6*ones(Ndt_d8,1);g2/2*ones(Ndt_d4,1)]*unit2si;
G.ref_z = [g7*ones(Ndt_d8,1);0*ones(Ndt_d4,1);0*ones(Ndt_d6,1); 0*ones(Ndt_p3half,1); 0*ones(Ndt_p3half,1);0*ones(Ndt_d6,1);g7*ones(Ndt_d8,1);0*ones(Ndt_d4,1)]*unit2si;

G.acq_x = [0*ones(Ndt_d9,1); 0*ones(Ndt_d24,1); 0*ones(Ndt_d7,1); 0*ones(Ndt_u20,1); g8*ones(Ndt_d10,1); 0*ones(Ndt_d4,1);0*ones(Ndt_d6,1); 0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);0*ones(Ndt_d6,1);-1*g8*ones(Ndt_d10,1);0*ones(Ndt_d11,1); 0*ones(Ndt_u10,1); 0*ones(Ndt_u10,1)]*unit2si;
G.acq_y = [0*ones(Ndt_d9,1); 0*ones(Ndt_d24,1); 0*ones(Ndt_d7,1); 0*ones(Ndt_u20,1); g9*ones(Ndt_d10,1); 0*ones(Ndt_d4,1);0*ones(Ndt_d6,1); 0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);0*ones(Ndt_d6,1);-1*g9*ones(Ndt_d10,1);0*ones(Ndt_d11,1); 0*ones(Ndt_u10,1); 0*ones(Ndt_u10,1)]*unit2si;
G.acq_z = [0*ones(Ndt_d9,1); 0*ones(Ndt_d24,1); 0*ones(Ndt_d7,1); 0*ones(Ndt_u20,1); g10*ones(Ndt_d10,1); 0*ones(Ndt_d4,1);g3*ones(Ndt_d6,1); g3*ones(Ndt_p4half,1); -1*g3*ones(Ndt_p4half,1);-1*g3*ones(Ndt_d6,1);-1*g10*ones(Ndt_d10,1);0*ones(Ndt_d11,1); 0*ones(Ndt_u10,1); 0*ones(Ndt_u10,1)]*unit2si;

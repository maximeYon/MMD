function xps = mdm_bruker_my_DOR_R12_EPI_NOW_PV360_acqus2xps(data_path, xps_fn, rps)
...
NotDiffExp = ReadPV360Param(data_path_pv, 'NotDiffExp') ;
%% EPI parameters
%Delay
...
G.ref_x_noCrush = [0*ones(Ndt_d9,1);0*ones(Ndt_d4,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_d4,1); 0*ones(Ndt_d9,1)]*unit2si;
G.ref_y_noCrush = [0*ones(Ndt_d9,1);0*ones(Ndt_d4,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_d4,1); 0*ones(Ndt_d9,1)]*unit2si;
G.ref_z_noCrush = [0*ones(Ndt_d9,1);g0*ones(Ndt_d4,1); g0*ones(Ndt_p1half,1); g0*ones(Ndt_p1half,1); 0*ones(Ndt_d4,1); g0*ones(Ndt_d9,1)]*unit2si;
...
for ind_image = 1:td1
    if DiffusionExperimentList(ind_image)==1
        ...
        if NotDiffExp(ind_image)==0
        G.im_x{ind_image} = [G.exc_x; zeros(Ndt_AddTE(ind_image),1); zeros(Ndt_DwGradDur(ind_image),1); G.ref_x_noCrush; zeros(Ndt_DwGradDur(ind_image),1); zeros(Ndt_AddTE(ind_image),1); G.acq_x];
        G.im_y{ind_image} = [G.exc_y; zeros(Ndt_AddTE(ind_image),1); zeros(Ndt_DwGradDur(ind_image),1); G.ref_y_noCrush; zeros(Ndt_DwGradDur(ind_image),1); zeros(Ndt_AddTE(ind_image),1); G.acq_y];
        G.im_z{ind_image} = [G.exc_z; zeros(Ndt_AddTE(ind_image),1); zeros(Ndt_DwGradDur(ind_image),1); G.ref_z_noCrush; zeros(Ndt_DwGradDur(ind_image),1); zeros(Ndt_AddTE(ind_image),1); G.acq_z]; 
        else
...
for ind_image = 1:td1
    ...
    Size_shape = size(ShapeStru.(['DWGS' num2str(nShape) 'R']),2);
    Size_shapeR = size(ShapeStru.(['DWGS' num2str(nShape) 'Rr']),2);
    if DiffusionExperimentList(ind_image)==1
        ...
        G.cr = ShapeStru.(['DWGS' num2str(nShape) 'Sr']);
         ...
         % Interp shape
        G.a = interp1(linspace(0,1,Size_shape)',ShapeStru.(['DWGS' num2str(nShape) 'R']),linspace(0,1,Ndt_DwD4(ind_image))','linear');
        G.b = interp1(linspace(0,1,Size_shape)',ShapeStru.(['DWGS' num2str(nShape) 'P']),linspace(0,1,Ndt_DwD4(ind_image))','linear');
        G.c = interp1(linspace(0,1,Size_shape)',ShapeStru.(['DWGS' num2str(nShape) 'S']),linspace(0,1,Ndt_DwD4(ind_image))','linear');
        G.ar = interp1(linspace(0,1,Size_shapeR)',ShapeStru.(['DWGS' num2str(nShape) 'Rr']),linspace(0,1,Ndt_DwD4(ind_image))','linear');
        G.br = interp1(linspace(0,1,Size_shapeR)',ShapeStru.(['DWGS' num2str(nShape) 'Pr']),linspace(0,1,Ndt_DwD4(ind_image))','linear');
        G.cr = interp1(linspace(0,1,Size_shapeR)',ShapeStru.(['DWGS' num2str(nShape) 'Sr']),linspace(0,1,Ndt_DwD4(ind_image))','linear');

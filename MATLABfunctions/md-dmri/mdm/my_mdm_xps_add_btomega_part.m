function xps = my_mdm_xps_add_btomega_part(xps_in,rps,Seq_part)
...
if display ==1; figure(10); end
for expN = 1:size(q.x,1)
    for part= 1:size(Seq_part,1)
        if part ==1
            x_tmp = fftshift(fft(q.x(expN,1:Seq_part(1,expN)),Nomega,2),2)*dt;
            y_tmp = fftshift(fft(q.y(expN,1:Seq_part(1,expN)),Nomega,2),2)*dt;
            z_tmp = fftshift(fft(q.z(expN,1:Seq_part(1,expN)),Nomega,2),2)*dt;
            
            sxx_tmp_tot = x_tmp.*(conj(x_tmp));
            syy_tmp_tot = y_tmp.*(conj(y_tmp));
            szz_tmp_tot = z_tmp.*(conj(z_tmp));
            sxy_tmp_tot = x_tmp.*(conj(y_tmp));
            sxz_tmp_tot = x_tmp.*(conj(z_tmp));
            syz_tmp_tot = y_tmp.*(conj(z_tmp));
            strace_tmp_tot = sxx_tmp_tot + syy_tmp_tot + szz_tmp_tot;
            if display ==1
                subplot(3,1,1)
                cla
                hold on
                plot(q.x(expN,1:Seq_part(1,expN)));
                plot(q.y(expN,1:Seq_part(1,expN)));
                plot(q.z(expN,1:Seq_part(1,expN)));
                subplot(3,1,2)
                cla
                hold on
                plot(real(sxx_tmp_tot)); plot(real(sxy_tmp_tot)); plot(real(sxz_tmp_tot))
                plot(real(syy_tmp_tot)); plot(real(syz_tmp_tot))
                plot(real(szz_tmp_tot))
                plot(real(strace_tmp_tot));
                xlim([size(strace_tmp_tot,2)/2-600 size(strace_tmp_tot,2)/2+600]);
                drawnow
                pause(1)
            end
            
        else
            x_tmp = fftshift(fft(q.x(expN,Seq_part(part-1,expN):Seq_part(part,expN)),Nomega,2),2)*dt;
            y_tmp = fftshift(fft(q.y(expN,Seq_part(part-1,expN):Seq_part(part,expN)),Nomega,2),2)*dt;
            z_tmp = fftshift(fft(q.z(expN,Seq_part(part-1,expN):Seq_part(part,expN)),Nomega,2),2)*dt;
            ...
            if display ==1
                ...
                cla
                hold on
                plot(q.x(expN,Seq_part(part-1,expN):Seq_part(part,expN)));
                plot(q.y(expN,Seq_part(part-1,expN):Seq_part(part,expN)));
                plot(q.z(expN,Seq_part(part-1,expN):Seq_part(part,expN)));
                subplot(3,1,2)
                cla
                hold on
                plot(real(sxx_tmp)); plot(real(sxy_tmp)); plot(real(sxz_tmp))
                plot(real(syy_tmp)); plot(real(syz_tmp))
                plot(real(szz_tmp))
                plot(real(strace_tmp));
                xlim([size(strace_tmp,2)/2-600 size(strace_tmp,2)/2+600]);
                subplot(3,1,3)
                cla
                hold on
                plot(real(sxx_tmp_tot)); plot(real(sxy_tmp_tot)); plot(real(sxz_tmp_tot))
            ...
            sxx_tmp_tot = sxx_tmp_tot + sxx_tmp;
            syy_tmp_tot = syy_tmp_tot + syy_tmp;
            szz_tmp_tot = szz_tmp_tot + szz_tmp;
            sxy_tmp_tot = sxy_tmp_tot + sxy_tmp;
            sxz_tmp_tot = sxz_tmp_tot + sxz_tmp;
            syz_tmp_tot = syz_tmp_tot + syz_tmp;
            strace_tmp_tot = strace_tmp_tot + strace_tmp;
        end
    end
    
    rmsomega(expN,:) = sqrt(sum(des.omega.^2.*strace_tmp_tot,2)./sum(strace_tmp_tot,2));
    
    % Truncate at max frequency and remove redundant values at negative
    % frequencies
    des.sxx(expN,:) = sxx_tmp_tot(:,ind);
    des.syy(expN,:) = syy_tmp_tot(:,ind);
    des.szz(expN,:) = szz_tmp_tot(:,ind);
    des.sxy(expN,:) = sxy_tmp_tot(:,ind);
    des.sxz(expN,:) = sxz_tmp_tot(:,ind);
    des.syz(expN,:) = syz_tmp_tot(:,ind);
    des.strace(expN,:) = strace_tmp_tot(:,ind);
end

function xps = mdm_bruker_my_DOR_R12_SPIRAL_PV6_acqus2xps(data_path, xps_fn, rps)
...
%% Spiral parameters
%Delay
dGrad=ReadPV360Param(data_path_pv, 'GradWait')/1000;
dFq=ReadPV360Param(data_path_pv, 'FreqSwitch')/1000;
dRise=ReadPV360Param(data_path_pv, 'RiseT')/1000;
dAmp=ReadPV360Param(data_path_pv, 'AmpEnable')/1000;
SliceSpoil=ReadPV360Param(data_path_pv, 'SliceSpoiler');
SliceSpoil =  split(SliceSpoil,',');
dSpoil = str2double(SliceSpoil{3})/1000;
dReph=ReadPV360Param(data_path_pv, 'RephGradDur')/1000;
dRfc2=ReadPV360Param(data_path_pv, 'RfcSpoilDur')/1000;
dTE=ReadPV360Param(data_path_pv, 'TEWait')/1000;
dTErfc=ReadPV360Param(data_path_pv, 'TEWaitRfc')/1000;
dTR=ReadPV360Param(data_path_pv, 'TRWait')/1000;
dRfc1 = dRfc2 - dAmp;
%% Retrieve baselevel parameters
% grad val
SliceGrad=ReadPV360Param(data_path_pv, 'SliceGrad');
SliceRephGrad=ReadPV360Param(data_path_pv, 'SliceRephGrad');
Phase3dGrad=ReadPV360Param(data_path_pv, 'Phase3dGrad');
RfcSpoilGrad=ReadPV360Param(data_path_pv, 'RfcSpoilGrad');
% pulse duration
p0 = Pulses(0+1)*1e-6;
p1 = Pulses(1+1)*1e-6;
...
EffecectiveTRcalc = dTR + AddTRList + 2* dFq + dGrad + dSpoil + dRise + dAmp +(p0*(1-ExcPulse1_Rpfac));
...
Ndt_dGrad = round(dGrad/dt);
Ndt_dFq = round(dFq/dt);
Ndt_dRise = round(dRise/dt);
Ndt_dAmp = round(dAmp/dt);
Ndt_dSpoil = round(dSpoil/dt);
Ndt_dReph = round(dReph/dt);
Ndt_dRfc1 = round(dRfc1/dt);
Ndt_dRfc2 = round(dRfc2/dt);
Ndt_dTE = round(dTE/dt);
Ndt_dTErfc = round(dTErfc/dt);
Ndt_dTR = round(dTR/dt);
Ndt_AddTE = round(AdditionalTESec/dt);
Ndt_DwD1 = round(DwD1/dt);

%% Compute main sequence part timing
Ndt_preref = Ndt_p0half + Ndt_dRise + Ndt_dReph + Ndt_dRise + Ndt_dGrad + Ndt_dTE +...
    Ndt_AddTE + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList  +...
    Ndt_dRfc1 + Ndt_dAmp + Ndt_dRise + Ndt_p1half;
Ndt_postref = Ndt_p1half + Ndt_dRfc2 + Ndt_dRise +...
    (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE +  ...
    Ndt_dGrad + Ndt_dTErfc;
Ndt_tot = Ndt_preref + Ndt_postref;
% offset = abs(Ndt_preref - Ndt_postref)*dt*1000; %offset

%% Create sequences part for b-calculation
Seq_part(1,:) = Ndt_p0half + Ndt_dRise + Ndt_dReph + Ndt_dRise + Ndt_dGrad + Ndt_dTE + Ndt_AddTE;
Seq_part(2,:) = Seq_part(1,:) + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList;
Seq_part(3,:) = Seq_part(2,:) + Ndt_dRfc1 + Ndt_dAmp + Ndt_dRise + Ndt_p1half + Ndt_p1half + Ndt_dRfc2 + Ndt_dRise;
Seq_part(4,:) = Seq_part(3,:) + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE;
Seq_part(5,:) = Seq_part(4,:) + Ndt_dGrad + Ndt_dTErfc;
...
G.exc_x = [0*ones(Ndt_p0half,1); 0*ones(Ndt_dRise,1); 0*ones(Ndt_dReph,1); 0*ones(Ndt_dRise,1); 0*ones(Ndt_dGrad,1); 0*ones(Ndt_dTE,1)]*unit2si;
G.exc_y = [0*ones(Ndt_p0half,1); 0*ones(Ndt_dRise,1); 0*ones(Ndt_dReph,1); 0*ones(Ndt_dRise,1); 0*ones(Ndt_dGrad,1); 0*ones(Ndt_dTE,1)]*unit2si;
G.exc_z = [SliceGrad*ones(Ndt_p0half,1); SliceGrad/2*ones(Ndt_dRise,1); -SliceRephGrad*ones(Ndt_dReph,1); 0*ones(Ndt_dRise,1); 0*ones(Ndt_dGrad,1); 0*ones(Ndt_dTE,1)]*unit2si;

G.ref_x = [0*ones(Ndt_dRfc1,1);0*ones(Ndt_dAmp,1);0*ones(Ndt_dRise,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_dRfc2,1); 0*ones(Ndt_dRise,1)]*unit2si;
G.ref_y = [0*ones(Ndt_dRfc1,1);0*ones(Ndt_dAmp,1);0*ones(Ndt_dRise,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_dRfc2,1); 0*ones(Ndt_dRise,1)]*unit2si;
G.ref_z = [RfcSpoilGrad*ones(Ndt_dRfc1,1);RfcSpoilGrad*ones(Ndt_dAmp,1);SliceGrad*ones(Ndt_dRise,1); SliceGrad*ones(Ndt_p1half,1); SliceGrad*ones(Ndt_p1half,1); RfcSpoilGrad*ones(Ndt_dRfc2,1); SliceGrad*ones(Ndt_dRise,1)]*unit2si;

G.acq_x = [0*ones(Ndt_dGrad,1); 0*ones(Ndt_dTErfc,1)]*unit2si;
G.acq_y = [0*ones(Ndt_dGrad,1); 0*ones(Ndt_dTErfc,1)]*unit2si;
G.acq_z = [0*ones(Ndt_dGrad,1); 0*ones(Ndt_dTErfc,1)]*unit2si;

function xps = mdm_bruker_my_DOR_R12_PRESS_e2_PV6_acqus2xps(data_path, xps_fn, rps)
...
VoxPul1=ReadPV360Param(data_path_pv, 'VoxPul1') ;
VoxPul1_Rpfac =  split(VoxPul1,',');
VoxPul1_Rpfac = str2double(VoxPul1_Rpfac{10})/100;
VoxPul2=ReadPV360Param(data_path_pv, 'VoxPul2') ;
VoxPul2_Rpfac =  split(VoxPul2,',');
VoxPul2_Rpfac = str2double(VoxPul2_Rpfac{10})/100;
VoxPul3=ReadPV360Param(data_path_pv, 'VoxPul3') ;
VoxPul3_Rpfac =  split(VoxPul3,',');
VoxPul3_Rpfac = str2double(VoxPul3_Rpfac{10})/100;
...
u1018 = 0.001018;
EffecectiveTRcalc = d1+u1018+AddTRList+d4+d6+(p1*(1-VoxPul1_Rpfac));
...
Ndt_d6 = round(d6/dt);
Ndt_d7 = round(d7/dt);
Ndt_d7minusd4 = round(Ndt_d7-Ndt_d4);
Ndt_d8 = round(d8/dt);
Ndt_d9 = round(d9/dt);
Ndt_d10 = round(d10/dt);
Ndt_d11 = round(d11/dt);
Ndt_d14 = round(d14/dt);

Ndt_p1half = round(p1*VoxPul1_Rpfac/dt);
Ndt_p3half = round(p3/2/dt);
Ndt_p4half = round(p4/2/dt);
Ndt_de4 = round(de4/dt);
Ndt_u20 = round(u20/dt);
Ndt_u10 = round(u10/dt);
Ndt_aqqhalf = round(aqq/2/dt);
Ndt_AddTE = round(AdditionalTESec/dt);
Ndt_DwD1 = round(DwD1/dt);

%% Compute main sequence part timing
Ndt_preref = Ndt_p1half + Ndt_d6 + Ndt_d4 + Ndt_d7minusd4 + ...
    Ndt_d8 + Ndt_d4 + Ndt_d6 + Ndt_p3half + ...
    Ndt_p3half + Ndt_d6 + Ndt_d8 + Ndt_d4 + Ndt_d9 + Ndt_u20 +...
    Ndt_AddTE + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList  +...
    Ndt_d14 + Ndt_d10 + Ndt_d4 + Ndt_d6 + Ndt_p4half;
Ndt_postref = Ndt_p4half + Ndt_d6 + Ndt_d10 + Ndt_d4 +...
   (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE +  ...
    Ndt_d14 + Ndt_d11 + Ndt_u10 + Ndt_u10; % stop before aqq if FID (not echo acquistion)
Ndt_tot = Ndt_preref + Ndt_postref;

%% Create sequences part for b-calculation
Seq_part(1,:) = Ndt_p1half + Ndt_d6 + Ndt_d4 + Ndt_d7minusd4 + Ndt_d8 + Ndt_d4 + Ndt_d6 + Ndt_p3half + Ndt_p3half + Ndt_d6 + Ndt_d8 + Ndt_d4 + Ndt_d9 + Ndt_u20 + Ndt_AddTE;
Seq_part(2,:) = Seq_part(1,:) + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList;
Seq_part(3,:) = Seq_part(2,:) + Ndt_d14 + Ndt_d10 + Ndt_d4 + Ndt_d6 + Ndt_p4half + Ndt_p4half + Ndt_d6 + Ndt_d10 + Ndt_d4;
Seq_part(4,:) = Seq_part(3,:) + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE;
Seq_part(5,:) = Seq_part(4,:) + Ndt_d14 + Ndt_d11 + Ndt_u10 + Ndt_u10;
...
G.exc_x = [-1*g1*ones(Ndt_p1half,1);-1*g1*ones(Ndt_d6,1); -1*g1/2*ones(Ndt_d4,1); -1*0*ones(Ndt_d7minusd4,1);-1*g4*ones(Ndt_d8,1);-1*0*ones(Ndt_d4,1);-1*0*ones(Ndt_d6,1); -1*0*ones(Ndt_p3half,1); 0*ones(Ndt_p3half,1);0*ones(Ndt_d6,1);g5*ones(Ndt_d8,1);0*ones(Ndt_d4,1);0*ones(Ndt_d9,1); 0*ones(Ndt_u20,1)]*unit2si;
G.exc_y = [-1*0*ones(Ndt_p1half,1);-1*0*ones(Ndt_d6,1); -1*0*ones(Ndt_d4,1); -1*0*ones(Ndt_d7minusd4,1);-1*g6*ones(Ndt_d8,1);-1*g2/2*ones(Ndt_d4,1);-1*g2*ones(Ndt_d6,1);-1*g2*ones(Ndt_p3half,1);g2*ones(Ndt_p3half,1);g2*ones(Ndt_d6,1);g6*ones(Ndt_d8,1);g2/2*ones(Ndt_d4,1);0*ones(Ndt_d9,1); 0*ones(Ndt_u20,1)]*unit2si;
G.exc_z = [-1*0*ones(Ndt_p1half,1);-1*0*ones(Ndt_d6,1); -1*0*ones(Ndt_d4,1); -1*0*ones(Ndt_d7minusd4,1);-1*g7*ones(Ndt_d8,1);-1*0*ones(Ndt_d4,1);-1*0*ones(Ndt_d6,1); -1*0*ones(Ndt_p3half,1); 0*ones(Ndt_p3half,1);0*ones(Ndt_d6,1);g7*ones(Ndt_d8,1);0*ones(Ndt_d4,1);0*ones(Ndt_d9,1); 0*ones(Ndt_u20,1)]*unit2si;

G.ref_x = [0*ones(Ndt_d14,1);g8*ones(Ndt_d10,1); 0*ones(Ndt_d4,1);0*ones(Ndt_d6,1); 0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);0*ones(Ndt_d6,1);g8*ones(Ndt_d10,1);0*ones(Ndt_d4,1)]*unit2si;
G.ref_y = [0*ones(Ndt_d14,1);g9*ones(Ndt_d10,1); 0*ones(Ndt_d4,1);0*ones(Ndt_d6,1); 0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);0*ones(Ndt_d6,1);g9*ones(Ndt_d10,1);0*ones(Ndt_d4,1)]*unit2si;
G.ref_z = [0*ones(Ndt_d14,1);g10*ones(Ndt_d10,1); 0*ones(Ndt_d4,1);g3*ones(Ndt_d6,1); g3*ones(Ndt_p4half,1); g3*ones(Ndt_p4half,1);g3*ones(Ndt_d6,1);g10*ones(Ndt_d10,1);0*ones(Ndt_d4,1)]*unit2si;

G.acq_x = [0*ones(Ndt_d14,1);0*ones(Ndt_d11,1); 0*ones(Ndt_u10,1); 0*ones(Ndt_u10,1)]*unit2si;
G.acq_y = [0*ones(Ndt_d14,1);0*ones(Ndt_d11,1); 0*ones(Ndt_u10,1); 0*ones(Ndt_u10,1)]*unit2si;
G.acq_z = [0*ones(Ndt_d14,1);0*ones(Ndt_d11,1); 0*ones(Ndt_u10,1); 0*ones(Ndt_u10,1)]*unit2si;
...
for ind_image = 1:td1
    ...
    G.x(1:Ndt_preref(ind_image)-1) = -G.x(1:Ndt_preref(ind_image)-1);
    G.y(1:Ndt_preref(ind_image)-1) = -G.y(1:Ndt_preref(ind_image)-1);
    G.z(1:Ndt_preref(ind_image)-1) = -G.z(1:Ndt_preref(ind_image)-1);

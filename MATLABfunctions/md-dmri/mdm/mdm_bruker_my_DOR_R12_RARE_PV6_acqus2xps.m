function xps = mdm_bruker_my_DOR_R12_RARE_PV6_acqus2xps(data_path, xps_fn, rps)
...
if strcmp(VFA,'yes')==1
    ...
    if strcmp(Method_name,'<user:my_dorr12_rare_final>')==1
...
if strcmp(BIRRefoc,'yes')==1
    Ndt_postref = Ndt_p3 + Ndt_p4 + Ndt_d3 +...
...
if strcmp(VFA,'yes')==1
    ...
    Spin_echo_dur = Spin_echo_dur_right + Spin_echo_dur_left; % s
    Inter_RARE_echo_delay = (Ndt_d3 + 2*Ndt_p1half + Ndt_d5 + Ndt_d6 + Ndt_denab + Ndt_Aq + Ndt_d6 + Ndt_d5)*dt; % s
...
G.exc_z = [g0_exc*ones(Ndt_p0half,1); g0/2*ones(Ndt_d3,1); g12*ones(Ndt_d12,1); 0*ones(Ndt_d3,1)]*unit2si;

...
b    = cell2mat(cellfun(@(x) str2num(x), bval, 'uniformoutput', 0))' * 1e6;

if (isempty(bvec_fn)), return; end

bvec = mdm_txt_read(bvec_fn);
u = cell2mat(cellfun(@(x) str2num(x), bvec, 'uniformoutput', 0));

assert(3*numel(b) == numel(u), 'strange bvec file');

% Extract and normalize the diffusion encoding directions
u = reshape(u, 3, numel(b))';

function xps = mdm_bruker_my_DOR_R12_SegFLASH_PV360_acqus2xps(data_path, xps_fn, rps)
%
...
ExcDiffPulse=ReadPV360Param(data_path_pv, 'ExcDiffPulse') ;
ExcDiffPulse_Rpfac =  split(ExcDiffPulse,',');
ExcDiffPulse_Rpfac = str2double(ExcDiffPulse_Rpfac{10})/100;
...
g1 = gradAmpl(1+1);
g2 = gradAmpl(2+1);
%g3 = 0; % force k-space center
%g4 = 0; % force k-space center
g5 = gradAmpl(5+1);
...
u31 = 0.000031;
EffecectiveTRcalc = d1+AddTRList+u31+(p3*(1-ExcDiffPulse_Rpfac));
...
Ndt_p3half = round(p3*ExcDiffPulse_Rpfac/dt);  
...
g0_exc = - g1*Ndt_d10/(Ndt_p0half+Ndt_d4/2);
%% Asociate delays with gradients
G.exc_x = [0*ones(Ndt_p3half,1)]*unit2si;
G.exc_y = [0*ones(Ndt_p3half,1)]*unit2si;
G.exc_z = [0*ones(Ndt_p3half,1)]*unit2si;

if strcmp(RefoCrusher,'yes')==1
    G.ref_x = [0*ones(Ndt_d22,1);0*ones(Ndt_d3,1);0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);0*ones(Ndt_d22,1); 0*ones(Ndt_d3,1);]*unit2si;
    G.ref_y = [0*ones(Ndt_d22,1);0*ones(Ndt_d3,1);0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);0*ones(Ndt_d22,1); 0*ones(Ndt_d3,1);]*unit2si;
    G.ref_z = [g22*ones(Ndt_d22,1);g22/2*ones(Ndt_d3,1);0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);g22*ones(Ndt_d22,1); g22/2*ones(Ndt_d3,1);]*unit2si;
else
    G.ref_x = [0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);]*unit2si;
    G.ref_y = [0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);]*unit2si;
    G.ref_z = [0*ones(Ndt_p4half,1); 0*ones(Ndt_p4half,1);]*unit2si;
end

G.acq_x = [0*ones(Ndt_p3half,1)]*unit2si; 
G.acq_y = [0*ones(Ndt_p3half,1)]*unit2si;
G.acq_z = [0*ones(Ndt_p3half,1)]*unit2si;

G.acq_x = cat(1,G.acq_x,[0*ones(Ndt_p0half,1);0*ones(Ndt_d4,1); g2*ones(Ndt_d10,1); 0*ones(Ndt_d2,1);g5/2*ones(Ndt_d3,1);g5*ones(Ndt_dAqDiv2,1)]*unit2si);
G.acq_y = cat(1,G.acq_y,[0*ones(Ndt_p0half,1);0*ones(Ndt_d4,1); 0*ones(Ndt_d10,1); 0*ones(Ndt_d2,1);0*ones(Ndt_d3,1);0*ones(Ndt_dAqDiv2,1)]*unit2si);
G.acq_z = cat(1,G.acq_z,[g0_exc*ones(Ndt_p0half,1);g0_exc/2*ones(Ndt_d4,1); g1*ones(Ndt_d10,1); 0*ones(Ndt_d2,1);0*ones(Ndt_d3,1);0*ones(Ndt_dAqDiv2,1)]*unit2si);

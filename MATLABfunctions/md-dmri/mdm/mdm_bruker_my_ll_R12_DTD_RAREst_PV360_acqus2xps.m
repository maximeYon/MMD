function xps = mdm_bruker_my_ll_R12_DTD_RAREst_PV360_acqus2xps(data_path, xps_fn, rps)
...
% Gradients
ReadShape=ReadPV360Param(data_path_pv, 'ReadShape') ;
PhaseShape=ReadPV360Param(data_path_pv, 'PhaseShape') ;
SliceShape=ReadPV360Param(data_path_pv, 'SliceShape') ;
...
for nShape = 1:DwNShapes
    ...
    % Interp shape
    ShapeStru.(['DWGS' num2str(nShape-1) 'R']) = interp1(linspace(0,1,numel(ShapeStru.(['DWGS' num2str(nShape-1) 'R'])))',ShapeStru.(['DWGS' num2str(nShape-1) 'R']),linspace(0,1,Ndt_DwD4)','makima');
    ShapeStru.(['DWGS' num2str(nShape-1) 'P']) = interp1(linspace(0,1,numel(ShapeStru.(['DWGS' num2str(nShape-1) 'P'])))',ShapeStru.(['DWGS' num2str(nShape-1) 'P']),linspace(0,1,Ndt_DwD4)','makima');
    ShapeStru.(['DWGS' num2str(nShape-1) 'S']) = interp1(linspace(0,1,numel(ShapeStru.(['DWGS' num2str(nShape-1) 'S'])))',ShapeStru.(['DWGS' num2str(nShape-1) 'S']),linspace(0,1,Ndt_DwD4)','makima');
end
...
Ndt_d4 = round(d4/dt);
Ndt_d8 = round(d8/dt);
Ndt_d10 = round(d10/dt);
Ndt_d10div2 = round(Ndt_d10/2);
Ndt_d11 = round(d11/dt);
Ndt_AddTE = round(AdditionalTESec/dt);
Ndt_DwD1 = round(DwD1/dt);

%% Interpolate RAREst shapes
ReadShape= interp1(0:(Ndt_d10-1)/(size(ReadShape,2)-1):(Ndt_d10-1),ReadShape,0:(Ndt_d10-1),'linear');%
ReadShape= ReadShape'.*100;
ReadShapeDiv2 = ReadShape(1:Ndt_d10div2);
PhaseShape= interp1(0:(Ndt_d10-1)/(size(PhaseShape,2)-1):(Ndt_d10-1),PhaseShape,0:(Ndt_d10-1),'linear');%
PhaseShape= PhaseShape'.*100;
PhaseShapeDiv2 = PhaseShape(1:Ndt_d10div2);
SliceShape= interp1(0:(Ndt_d10-1)/(size(SliceShape,2)-1):(Ndt_d10-1),SliceShape,0:(Ndt_d10-1),'linear');%
SliceShape= SliceShape'.*100;
SliceShapeDiv2 = SliceShape(1:Ndt_d10div2);

%% Compute main sequence part timing
Ndt_preref = Ndt_p0half + Ndt_d4 +...
    Ndt_AddTE + (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList  +...
    Ndt_d4 + Ndt_d8 + Ndt_p1half;
Ndt_postref = Ndt_p1half + Ndt_d8 + Ndt_d4 +...
    (Ndt_DwD1 + Ndt_DwD4 +Ndt_DwD1).*DiffusionExperimentList + Ndt_AddTE +  ...
    Ndt_d4 + Ndt_d11 + Ndt_d2 + Ndt_d4 +...     
    (Ndt_d8 + 2*Ndt_p1half + Ndt_d4 + Ndt_d8 + Ndt_d3 + Ndt_d10).*(NSpinEcho-1) +... % loop
    Ndt_d8 + 2*Ndt_p1half  + Ndt_d4 + Ndt_d8 + Ndt_d3 + Ndt_d10div2;
...
G.exc_x = [0*ones(Ndt_p0half,1); 0*ones(Ndt_d4,1)]*unit2si;
G.exc_y = [0*ones(Ndt_p0half,1); 0*ones(Ndt_d4,1)]*unit2si;
G.exc_z = [g0*ones(Ndt_p0half,1); g0/2*ones(Ndt_d4,1)]*unit2si;

G.ref_x = [0*ones(Ndt_d4,1);0*ones(Ndt_d8,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_d8,1); 0*ones(Ndt_d4,1)]*unit2si;
G.ref_y = [0*ones(Ndt_d4,1);0*ones(Ndt_d8,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_p1half,1); 0*ones(Ndt_d8,1); 0*ones(Ndt_d4,1)]*unit2si;
G.ref_z = [g3/2*ones(Ndt_d4,1);g3*ones(Ndt_d8,1); g3*ones(Ndt_p1half,1); g3*ones(Ndt_p1half,1); g3*ones(Ndt_d8,1); g3/2*ones(Ndt_d4,1)]*unit2si;

% ReadShape = g2*d2/d10; %Ugly fix
% PhaseShape = 0 ;
% SliceShape = 0;
G.acq_x = [0*ones(Ndt_d4,1); 0*ones(Ndt_d11,1); g2*ones(Ndt_d2,1);0*ones(Ndt_d4,1)]*unit2si;
G.acq_y = [0*ones(Ndt_d4,1); 0*ones(Ndt_d11,1); 0*ones(Ndt_d2,1);0*ones(Ndt_d4,1)]*unit2si;
G.acq_z = [0*ones(Ndt_d4,1); 0*ones(Ndt_d11,1); -g1*ones(Ndt_d2,1);g3/2*ones(Ndt_d4,1)]*unit2si;
...
G.acq_x = cat(1,G.acq_x,[SignBpulse*0*ones(Ndt_d8,1);SignBpulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_d4,1);SignApulse*0*ones(Ndt_d8,1);SignApulse*0*ones(Ndt_d3,1);SignApulse*ReadShape.*ones(Ndt_d10,1)]*unit2si);
G.acq_y = cat(1,G.acq_y,[SignBpulse*0*ones(Ndt_d8,1);SignBpulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_d4,1);SignApulse*0*ones(Ndt_d8,1);SignApulse*0*ones(Ndt_d3,1);SignApulse*PhaseShape.*ones(Ndt_d10,1)]*unit2si);
G.acq_z = cat(1,G.acq_z,[SignBpulse*g3*ones(Ndt_d8,1);SignBpulse*g3*ones(Ndt_p1half,1);SignApulse*g3*ones(Ndt_p1half,1);SignApulse*g3/2*ones(Ndt_d4,1);SignApulse*0*ones(Ndt_d8,1);SignApulse*0*ones(Ndt_d3,1);SignApulse*SliceShape.*ones(Ndt_d10,1)]*unit2si);
end
ind_echo = ind_echo+1;
    if  mod(ind_echo,2) % Odd
    SignBpulse = 1; SignApulse = -1;
...
G.acq_x = cat(1,G.acq_x,[SignBpulse*0*ones(Ndt_d8,1);SignBpulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_d4,1);SignApulse*0*ones(Ndt_d8,1);SignApulse*0*ones(Ndt_d3,1);SignApulse.*ReadShapeDiv2.*ones(Ndt_d10div2,1)]*unit2si);
G.acq_y = cat(1,G.acq_y,[SignBpulse*0*ones(Ndt_d8,1);SignBpulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_p1half,1);SignApulse*0*ones(Ndt_d4,1);SignApulse*0*ones(Ndt_d8,1);SignApulse*0*ones(Ndt_d3,1);SignApulse*PhaseShapeDiv2.*ones(Ndt_d10div2,1)]*unit2si);
G.acq_z = cat(1,G.acq_z,[SignBpulse*g3*ones(Ndt_d8,1);SignBpulse*g3*ones(Ndt_p1half,1);SignApulse*g3*ones(Ndt_p1half,1);SignApulse*g3/2*ones(Ndt_d4,1);SignApulse*0*ones(Ndt_d8,1);SignApulse*0*ones(Ndt_d3,1);SignApulse*SliceShapeDiv2.*ones(Ndt_d10div2,1)]*unit2si);
...
for ind_image = 1:td1
    if DiffusionExperimentList(ind_image)==1
        ShapeNdtPos(ind_image,:) = [size(G.exc_x,1)+Ndt_AddTE(ind_image)+Ndt_DwD1+1 size(G.exc_x,1)+Ndt_AddTE(ind_image)+Ndt_DwGradDur+size(G.ref_x,1)+Ndt_DwD1+1];
...
xps_temp.gwf_t = repmat(dt*(1:1:Ndt_DwD4),[td1 1]);
